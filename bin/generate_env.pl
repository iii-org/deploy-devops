...
if (lc($Y_N) ne 'n') {
	...
	$env_template =~ s/{{deploy_mode}}/$ask_deploy_mode/g;
	$env_template =~ s/{{iiidevops_ver}}/$ask_iiidevops_ver/g;
	$env_template =~ s/{{vm1_ip}}/$ask_vm1_ip/g;
	$env_template =~ s/{{vm2_ip}}/$ask_vm2_ip/g;
	$env_template =~ s/{{gitlab_domain_name}}/$ask_gitlab_domain_name/g;
	$env_template =~ s/{{harbor_domain_name}}/$ask_harbor_domain_name/g;
	$env_template =~ s/{{redmine_domain_name}}/$ask_redmine_domain_name/g;
	$env_template =~ s/{{sonarqube_domain_name}}/$ask_sonarqube_domain_name/g;
	$env_template =~ s/{{iiidevops_domain_name}}/$ask_iiidevops_domain_name/g;
	$env_template =~ s/{{gitlab_root_password}}/$ask_gitlab_root_password/g;
	$env_template =~ s/{{gitlab_private_token}}/$ask_gitlab_private_token/g;
	$env_template =~ s/{{rancher_admin_password}}/$ask_rancher_admin_password/g;
	$env_template =~ s/{{redmine_admin_password}}/$ask_redmine_admin_password/g;
	$env_template =~ s/{{redmine_api_key}}/$ask_redmine_api_key/g;
	$env_template =~ s/{{sonarqube_admin_passwd}}/$ask_sonarqube_admin_passwd/g;
	$env_template =~ s/{{sonarqube_admin_token}}/$ask_sonarqube_admin_token/g;
	$env_template =~ s/{{harbor_admin_password}}/$ask_harbor_admin_password/g;
	$env_template =~ s/{{auto_password}}/$ask_auto_password/g;
	$env_template =~ s/{{random_key}}/$ask_random_key/g;
	$env_template =~ s/{{checkmarx_origin}}/$ask_checkmarx_origin/g;
	$env_template =~ s/{{checkmarx_username}}/$ask_checkmarx_username/g;
	$env_template =~ s/{{checkmarx_password}}/$ask_checkmarx_password/g;
	$env_template =~ s/{{checkmarx_secret}}/$ask_checkmarx_secret/g;	
	$env_template =~ s/{{webinspect_base_url}}/$ask_webinspect_base_url/g;
	$env_template =~ s/{{admin_init_login}}/$ask_admin_init_login/g;
	$env_template =~ s/{{admin_init_email}}/$ask_admin_init_email/g;
	$env_template =~ s/{{admin_init_password}}/$ask_admin_init_password/g;
