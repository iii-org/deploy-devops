...
$cmd = "ssh $ARGV[0] \"chmod a+x ~/add_k8s.sh; sudo -S perl ~/iiidevops_install.pl local; showmount -e $nfs_ip; sudo -S ~/add-insecure-registries.pl $harbor_ip $harbor_domain_name; sudo -S ~/add_k8s.sh\"";
