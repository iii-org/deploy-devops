...
$harbor_cert = "/data/harbor/cert/$harbor_url.crt";
if (!-e $harbor_cert) {
...
$cmd = "ssh $ARGV[0] \"showmount -e $nfs_ip; sudo -S cp ~/$harbor_url.crt /usr/local/share/ca-certificates/; sudo update-ca-certificates; sudo systemctl restart docker.service; ls /etc/ssl/certs | awk /$harbor_url/\"";
log_print("-----\n$cmd\n");
$cmd_msg = `$cmd`;
log_print("-----\n$cmd_msg\n\n");

# Check remote k8s node info
$nfs_check = (index($cmd_msg, '/iiidevopsNFS *')<0)?"ERROR!":"OK!";
$harbor_cert_check = (index($cmd_msg, "$harbor_url.pem")<0)?"ERROR!":"OK!";
